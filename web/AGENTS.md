...
- **插槽式布局不得夹带其它子节点**：`SectionPageLayout`（`src/components/layout/components/section-page-layout.tsx`）等插槽式布局只渲染 `Title` / `Actions` / `Content` / `Breadcrumb` 四种插槽的直接子节点，**其余直接子节点会被静默丢弃——不渲染、无警告**。弹窗（Dialog / Sheet / Drawer 等传送门组件）、隐藏字段等必须放在布局之外（用 Fragment 包裹整页）或 `Content` 插槽内部。典型症状：按钮点击后无报错、state 已变化但弹窗不出现，即子节点被插槽式布局丢弃（布局在 DEV 模式会输出 `[SectionPageLayout]` console.warn）。涉及"按钮打开弹窗"的页面必须有测试覆盖整页渲染后点击按钮的路径，仅以 `open` 直接渲染弹窗的测试覆盖不到该链路。
...
- **2026-09-13**：在 3.3 中补充「插槽式布局不得夹带其它子节点」：弹窗等组件不得作为 `SectionPageLayout` 直接子节点（会被静默丢弃），并要求弹窗打开链路有整页级测试覆盖。
