...
export async function getRegistrationCodes(
  ...
  const res = await api.get(
    `/api/registration-code/?p=${p}&page_size=${page_size}`
  )
  return res.data
}
...
export async function updateRegistrationCodeStatus(
  ...
  const res = await api.put('/api/registration-code/?status_only=true', {
    id,
    status,
  })
  return res.data
...
// Delete invalid registration codes (used, disabled, expired)
