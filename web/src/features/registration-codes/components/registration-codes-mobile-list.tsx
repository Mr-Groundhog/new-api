...
import { REGISTRATION_STATUS, REGISTRATION_STATUSES } from '../constants'
  ...
  return (
    <div className='divide-border overflow-hidden rounded-lg border'>
      {rows.map((row) => {
        ...
        const expired = isRegistrationExpired(code.expired_time, code.status)
        const statusConfig = REGISTRATION_STATUSES[code.status]
        ...
        return (
          <div
            ...
            className={cn(
              ...
              expired || code.status !== REGISTRATION_STATUS.ENABLED
                ? DISABLED_ROW_MOBILE
                : undefined
            )}
          >
