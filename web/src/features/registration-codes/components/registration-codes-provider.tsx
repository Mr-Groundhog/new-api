...
export function RegistrationCodesProvider({
  ...
  const [open, setOpen] = useDialogState<RegistrationCodesDialogType>(null)
...
export const useRegistrationCodes = () => {
  const registrationCodesContext = React.useContext(RegistrationCodesContext)

  if (!registrationCodesContext) {
    throw new Error(
      'useRegistrationCodes has to be used within <RegistrationCodesProvider>'
    )
  }

  return registrationCodesContext
}
