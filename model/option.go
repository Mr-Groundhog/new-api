...
func InitOptionMap() {
	...
	common.OptionMap["ModelRequestClientRestrictionEnabled"] = strconv.FormatBool(setting.ModelRequestClientRestrictionEnabled)
...
func updateOptionMap(key string, value string) (err error) {
	...
	if strings.HasSuffix(key, "Enabled") || key == "DefaultCollapseSidebar" || key == "DefaultUseAutoGroup" || key == "SMTPForceAuthLogin" || key == "SMTPInsecureSkipVerify" {
		...
		case "ModelRequestClientRestrictionEnabled":
			setting.ModelRequestClientRestrictionEnabled = boolValue
